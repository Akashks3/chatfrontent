...
const GroupChatBox = () => {
	...
	return (
		<div className="flex -m-2 sm:-m-4 flex-col items-center my-6 text-slate-300 min-h-screen w-full fixed top-0 justify-center z-50">
			<div className="p-3 pt-4 w-[80%] sm:w-[60%] md:w-[50%] lg:w-[40%] min-w-72 max-w-[1000px] bg-white text-black rounded-lg h-fit mt-5 transition-all relative">
				<h2 className="text-2xl  font-semibold text-black w-full text-center mb-2">
					Create a Group
				</h2>
				<div className="w-full py-4 justify-evenly flex flex-wrap items-center gap-3">
					<div className="w-full flex flex-nowrap items-center justify-center gap-2">
						<input
							...
							className="w-2/3  py-1 px-2 font-normal outline-none rounded-md cursor-pointer  border border-slate-600 active:bg-black/20"
						...
						{isGroupUsers?.length != 0 &&
							isGroupUsers?.map((user) => {
								return (
									<div
										key={user?._id}
										className="flex justify-center text-black border border-slate-600 items-center gap-1 py-1 px-2 font-normal rounded-md cursor-pointer bg-transparent active:bg-black/20 text-nowrap"
									>
							...
							<>
								...
								{selectedUsers?.map((user) => {
									return (
										<div
											key={user?._id}
											className="w-full h-16 border-slate-500 border rounded-lg flex justify-start items-center p-2 font-semibold gap-2 text-black hover:bg-black/50 transition-all cursor-pointer"
											onClick={() => {
												addGroupUser(user);
												setInputUserName("");
											}}
										>
											<img
												className="h-12 min-w-12 rounded"
												src={user?.image}
												alt="img"
											/>
				...
				<div className="w-full flex flex-nowrap items-center justify-center gap-2">
					<input
						...
						className="w-2/3 border border-slate-600 py-1 px-2 font-normal outline-none rounded-md cursor-pointer text-black bg-transparent active:bg-black/20"
						onChange={(e) => setGroupName(e.target?.value)}
					/>
					<button
						className=" py-1 px-2 rounded-lg bg-green-400 text-black font-semibold hover:text-white hover:bg-green-700 "
