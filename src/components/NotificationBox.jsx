...
const NotificationBox = () => {
	...
	return (
		<div className="flex -m-2 sm:-m-4 flex-col items-center my-6 text-black min-h-screen w-full fixed top-0 justify-center z-50">
			<div className="p-3 pt-4 w-[80%] sm:w-[60%] md:w-[50%] lg:w-[40%] min-w-72 bg-white  max-w-[1000px] rounded-lg h-fit mt-5 transition-all relative">
							...
							<>
								...
								{newMessageRecieved?.map((message) => {
									return (
										<div
											key={message?._id}
											className="w-full h-16  rounded-lg flex justify-start items-center p-2 font-normal gap-2 hover:bg-black/50 transition-all cursor-pointer text-white"
											onClick={() => {
												dispatch(
													removeNewMessageRecieved(
														message
